...
# docker
[ "$ENABLE_DOCKER" = "y" ] && cat ../configs/config-docker >> .config

# ShadowSocksR Plus
[ "$ENABLE_SSRP" = "y" ] && cat ../configs/config-ssrp >> .config

# passwall
[ "$ENABLE_PASSWALL" = "y" ] && cat ../configs/config-passwall >> .config

# nikki
[ "$ENABLE_NIKKI" = "y" ] && cat ../configs/config-nikki >> .config

# openclash
[ "$ENABLE_OPENCLASH" = "y" ] && cat ../configs/config-openclash >> .config

# AdGuard Home
# [ "$ENABLE_ADGUARDHOME" = "y" ] && cat ../configs/config-adguardhome >> .config

# Lucky
[ "$ENABLE_LUCKY" = "y" ] && cat ../configs/config-lucky >> .config

# OpenAppFilter
[ "$ENABLE_OAF" = "y" ] && cat ../configs/config-oaf >> .config

# MosDNS
# [ "$ENABLE_MOSDNS" = "y" ] && cat ../configs/config-mosdns >> .config

# OpenList
# [ "$ENABLE_OPENLIST" = "y" ] && cat ../configs/config-openlist >> .config

# 任务设置
# [ "$ENABLE_TASKPLAN" = "y" ] && cat ../configs/config-taskplan >> .config

# 高级设置
# [ "$ENABLE_ADVANCEDPLUS" = "y" ] && cat ../configs/config-advancedplus >> .config
...
patch -p1 < ../doc/patch/kernel-6.6/kernel/0003-include-kernel-defaults.mk.patch

# bbr
cp ../doc/patch/kernel-6.6/bbr3/*.patch target/linux/generic/backport-6.6/
...
patch -p1 < ../Customize/frpc/001-luci-app-frpc-hide-token.patch
patch -p1 < ../Customize/frpc/002-luci-app-frpc-add-enable-flag.patch
...
# popd
pushd feeds/luci
    patch -p1 < ../../Customize/natmap/0001-luci-app-natmap-add-default-STUN-server-lists.patch
popd
